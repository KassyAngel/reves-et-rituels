...
function PlantCard({
  ...
  const locked = LOCKED_PLANTS.has(plant.id) && !isUnlocked;
  return (
    <motion.div
...
export default function Plants() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {/* ════════════════════════════════════════════════
          ÉCRAN PRINCIPAL — grille des plantes
      ════════════════════════════════════════════════ */}
      <div
      ...
      >
        <div className="px-4 pb-10 pt-2">
          ...
          {/* Filtres catégories */}
          <div className="flex gap-2 overflow-x-auto pb-3 mb-6 snap-x" style={{ scrollbarWidth: "none" }}>
            <button
              ...
              {filtered.map((plant, i) => (
                ...
                >
                  <PlantCard
                    ...
                    isUnlocked={isUnlocked(plant.id)}
                  />
                </motion.div>
              ))}
            </AnimatePresence>
            ...
            {showLocked ? (
              ...
              <>
                <div className="relative w-full flex-shrink-0" style={{ height: 200 }}>
                  <div className={`w-full h-full bg-gradient-to-br ${categoryColors[selected.category]}`}>
                    <img
                  ...
                  <div className="absolute bottom-3 left-0 right-0 flex flex-col items-center gap-1 px-12">
                    ...
                    <p className="text-[11px] text-white/70 italic">{selected.latin}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
                ...
                <div className={`relative w-full bg-gradient-to-br ${categoryColors[selected.category]}`} style={{ height: 220 }}>
                  <img
                    src={`${import.meta.env.BASE_URL}${selected.image.replace(/^\//, "")}`}
                    alt={selected.name[lang as "fr" | "en"]}
                    className="w-full h-full object-cover"
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/65" />
                  <div className="absolute bottom-4 left-0 right-0 flex flex-col items-center gap-1 px-12">
                    <h2 className="font-bold text-lg text-white drop-shadow text-center leading-snug">
                      {selected.name[lang as "fr" | "en"]}
                    </h2>
                    <p className="text-[11px] text-white/75 italic">{selected.latin}</p>
                  </div>
                  {selected.warning && (
                    <div className="absolute top-4 right-4 w-7 h-7 bg-amber-400 rounded-full flex items-center justify-center">
                ...
                <div className="px-4 pt-4 pb-10 space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground text-center italic px-2">
                    {selected.tagline[lang as "fr" | "en"]}
                  </p>
                  ...
                  {selected.warning && (
                    <motion.div
                      ...
                      className="p-3 bg-amber-50 rounded-xl border border-amber-200 flex gap-2 items-start"
                    >
                      <AlertTriangle size={14} className="text-amber-500 shrink-0 mt-0.5" />
                  ...
                  >
                    ...
                    <p className="text-sm text-foreground/80 leading-relaxed bg-green-50 rounded-2xl p-4 border border-green-100">
                  ...
                  >
                    ...
                    <p className="text-sm text-foreground/80 leading-relaxed bg-purple-50 rounded-2xl p-4 border border-purple-100 italic">
                  ...
                  <motion.div
                    ...
                    className="p-3 bg-amber-50 rounded-xl border border-amber-100 flex gap-2 items-start"
                  >
                    <AlertTriangle size={14} className="text-amber-500 shrink-0 mt-0.5" />
                    <p className="text-[10px] text-amber-700 leading-relaxed">
                      {lang === "fr"
